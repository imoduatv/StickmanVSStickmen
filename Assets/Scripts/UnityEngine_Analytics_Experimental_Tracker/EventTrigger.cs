...
namespace UnityEngine.Analytics.Experimental.Tracker
{
	...
	public class EventTrigger
	{
		...
		public bool Test(GameObject gameObject = null)
		{
			...
			{
				num2++;
				if (rule.Test(out bool error, out string message))
				{
					num++;
				}
				else if (error)
				{
					UnityEngine.Debug.LogWarningFormat("Event trigger rule {0}{2} is incomplete ({1}). Result is false.", num2, message, (!(gameObject == null)) ? $" on GameObject '{gameObject.name}'" : null);
				}
