	...
	public static class AnalyticsEvent
	{
		...
		public static AnalyticsResult Custom(string eventName, IDictionary<string, object> eventData = null)
		{
			...
			case AnalyticsResult.Ok:
				if (debugMode)
				{
					UnityEngine.Debug.LogFormat("Successfully sent '{0}' event (Result: '{1}').{2}", eventName, analyticsResult, empty);
				}
				break;
			case AnalyticsResult.TooManyItems:
			case AnalyticsResult.InvalidData:
				UnityEngine.Debug.LogErrorFormat("Failed to send '{0}' event (Result: '{1}').{2}", eventName, analyticsResult, empty);
				break;
			default:
				UnityEngine.Debug.LogWarningFormat("Unable to send '{0}' event (Result: '{1}').{2}", eventName, analyticsResult, empty);
				break;
			}
